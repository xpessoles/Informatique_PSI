...
et \texttt{tom}. La complexité de cette opération est donc un $O(k)$. Par sommation, on voit donc que 
\texttt{miroir\_it} a une complexité en $O(n^2)$.\\
...
Dans la version récursive, on montre par récurrence que la complexité est également un $O(n^2)$. Posons l'hypothèse de 
récurrence : $(\mathcal P _k)$ : si \texttt{mot} est de longueur $k$, \texttt{miroir\_rec (mot)} nécessite $O(k^2)$ 
opérations.\\
